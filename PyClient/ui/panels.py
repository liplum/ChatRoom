...
class panel(control, ABC):
    def __init__(self):
        super().__init__()
        self._elements: OrderedSet[CTRL] = OrderedSet()
        self._focused = False
        self.on_prop_changed.add(self._on_layout_changed)
        self._cur_focused: Optional[CTRL] = None
        self._elements_changed = event()
        self._focused_changed = event()
    ...
    def elements_changed(self) -> event:
        ...
        """
        return self._elements_changed

    @property
    def focused_changed(self) -> event:
        """
        Para 1:panel object
    ...
    @property
    def cur_focused(self) -> CTRL:
        return self._cur_focused

    @cur_focused.setter
    def cur_focused(self, value: CTRL):
...
class stack(panel):
    ...
    def draw_on(self, buf: buffer):
        ...
        if self.orientation == vertical:
            h = 0
            for i, elemt in enumerate(self.elements):
                h += elemt.height
                if i > 0:
